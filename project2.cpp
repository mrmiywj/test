...
void CPU::simulate() {
	...
	string filePrefix = prefix(filePath);
	while (true) {
		...
		if (fd.find(currentProcess) == fd.end()) {
			...
			string file = filePrefix+ currentProcess + ".mem";
...
class LRUMemory: MemoryBase {
	...
	set<reverseAccessType> reverseTime;
	int pgNum;
public:
...
bool LRUMemory::accessPage(long long time, string pgName) {
	...
	if (pgAccessTime[pgName] < time) {
		reverseTime.erase(reverseAccessElement(pgAccessTime[pgName],pgName));
		pgAccessTime[pgName] = time;
		reverseTime.insert(reverseAccessElement(time, pgName));
	}
	return true;
}
...
bool LRUMemory::kickOut(long long time) {
	...
	set<reverseAccessType>::iterator iter = reverseTime.begin();
	while(pgMarked[iter->second] || pgAvailTime[iter->second] > time)
	{
		iter++;
		if (iter == reverseTime.end())
			printerror("LRU reach the end of the set");
	}
	string pgName = iter->second;
...
bool myMemory::memoryFull()
{
	return (pgNum == globalPages);
}

bool myMemory::accessPage(long long time, string pgName)
{
	if (pageTime.find(pgName) == pageTime.end())
		return false;
	else if (pageTime[pgName] > time)
...
bool myMemory::insertPage(long long time, string pgName)
{
	pageTime[pgName] = time;
	pageMarked[pgName] = true;
	pages.insert(pgName);
...
bool myMemory::kickOut(long long time)
{
	set<string>::iterator iter = pages.begin();
	while(pageMarked[*iter] || pageTime[*iter] > time)
	{
		iter++;
		if (iter == pages.end())
			printerror("Reach the end of my memory model!");
	}
	string kickout = *iter;
	pageTime.erase(kickout);
	pageMarked.erase(kickout);
	pages.erase(kickout);
	return true;
}

void myMemory::markBusy(string pgName)
{
	pageMarked[pgName] = true;
}

void myMemory::unmarkBusy(string pgName)
{
	pageMarked[pgName] = false;
}
